...
import (
	...
	"github.com/shardul-d/kv-store/internal"
)

const (
	LOCKFILE   = "kvstore.lock"
	HINTS_FILE = "kvstore.hints"
)
...
func Init(cfg ...Config) (*KVStore, error) {
	// Set options.
	opts := DefaultOptions()
	for _, opt := range cfg {
	...
	go kvstore.RunCompaction(opts.compactInterval)

	// Spawn a goroutine which checks for the file size of the active file at periodic interval.
	go kvstore.ExamineFileSize(kvstore.opts.checkFileSizeInterval)

	// Spawn a goroutine which flushes the file to disk periodically.
	if kvstore.opts.syncInterval != nil {
		go kvstore.SyncFile(*opts.syncInterval)
	}
...
func (b *KVStore) Put(k string, val []byte) error {
	b.Lock()
	defer b.Unlock()
...
func (b *KVStore) PutEx(k string, val []byte, ex time.Duration) error {
	b.Lock()
	defer b.Unlock()
...
func (b *KVStore) Fold(fn func(k string) error) error {
	b.Lock()
	defer b.Unlock()
